...
class HttpSubfolderTestCase extends HttpTestCase
{
    ...
    public function setUp(): void
    {
        ...
        $process = new Process('php ' . $this->laravelAppPath . '/artisan_sub domain:update_env ' . $this->site1 . ' --domain_values=\'{"APP_NAME":"' .
            $this->siteAppName1 . '","DB_DATABASE":"' . $this->siteDbName1 . '"}\'');
        $process->run();

        $process = new Process('php ' . $this->laravelAppPath . '/artisan_sub domain:update_env ' . $this->site2 . ' --domain_values=\'{"APP_NAME":"' .
            $this->siteAppName2 . '","DB_DATABASE":"' . $this->siteDbName2 . '"}\'');
        $process->run();

        $process = new Process('php ' . $this->laravelAppPath . '/artisan_sub domain:update_env ' . $this->subSite1 . ' --domain_values=\'{"APP_NAME":"' .
            $this->subSiteAppName1 . '","DB_DATABASE":"' . $this->subSiteDbName1 . '"}\'');
        $process->run();
